// ignore_for_file: prefer_const_constructors, prefer_typing_uninitialized_variables, must_be_immutable, prefer_if_null_operators, curly_braces_in_flow_control_structures, avoid_print, constant_identifier_names, unused_element, unused_local_variable
...
class DForm1 extends StatefulWidget {
  var name;
  var price;
  DForm1({Key? key, required this.name, required this.price}) : super(key: key);

  @override
  _DForm1State createState() => _DForm1State();
}

class _DForm1State extends State<DForm1> {
  var type;
  List listOfFacts = [];
  ...
  Future insert() async {
    ...
    var result = await conn.query(
        'insert into normalorders ( phone, location, ordern, name, price, food, user, size) values (?, ?, ?, ?, ?, ?, ?, ?)',
        [
          phone.text,
          type1.toString(),
          ordern.text,
                ...
                child: Container(
                  ...
                  child: DropdownButton(
                    ...
                    items: listOfFacts.map((list) {
                      return DropdownMenuItem(
                        value: list['foodname'].toString(),
                        child: Text(list['foodname'].toString()),
                      );
                    }).toList(),
                  ),
                ),
